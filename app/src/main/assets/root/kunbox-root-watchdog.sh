...
while :; do
    ...
    # Android may freeze the Java scheduler for minutes. PID identity is the
    # authoritative crash signal, so suspend and resume cannot kill live Root.
    if [ "$ROOT_INVALID" -ne 0 ]; then
        CURRENT_SESSION="$(cat "$SESSION_FILE" 2>/dev/null)"
